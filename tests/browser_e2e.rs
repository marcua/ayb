...
async fn test_registration_ui(
    ...
    println!("[{:.2}s] Testing registration UI flow", start_time.elapsed().as_secs_f64());
    
    // Navigate to registration page
    let nav_start = std::time::Instant::now();
    BrowserHelpers::navigate_and_wait(
        page, 
        &format!("{}/register", server_url), 
    ).await?;
    println!("[{:.2}s] ✓ Registration page loaded successfully (took {:.2}s)", 
             start_time.elapsed().as_secs_f64(), nav_start.elapsed().as_secs_f64());

    // Test 1: Form validation with empty fields
    let empty_test_start = std::time::Instant::now();
    println!("[{:.2}s] Testing empty form validation", start_time.elapsed().as_secs_f64());
    
    let form_submitted = BrowserHelpers::submit_form(page).await?;
    assert!(form_submitted, "Should be able to find and submit registration form");
    ...
    println!("[{:.2}s] Empty form validation result: {}", 
             start_time.elapsed().as_secs_f64(), 
             if has_validation_error { "validation shown" } else { "no validation" });

    // Test 2: Valid registration data
    let fill_start = std::time::Instant::now();
    println!("[{:.2}s] Filling registration form with valid data", start_time.elapsed().as_secs_f64());
    BrowserHelpers::fill_registration_form(
        page, 
        "test@example.com", 
        "testuser"
    ).await?;
    println!("[{:.2}s] ✓ Form filled (took {:.2}s)", 
             start_time.elapsed().as_secs_f64(), fill_start.elapsed().as_secs_f64());
    
    let submit_start = std::time::Instant::now();
    println!("[{:.2}s] Submitting registration form", start_time.elapsed().as_secs_f64());
    let form_submitted = BrowserHelpers::submit_form(page).await?;
    assert!(form_submitted, "Should be able to submit registration form with valid data");
    ...
    match result {
        ...
        NavigationResult::ErrorDisplayed => {
            ...
            println!("[{:.2}s] Server error displayed for registration data (submit took {:.2}s)", 
                     start_time.elapsed().as_secs_f64(), submit_start.elapsed().as_secs_f64());
        },
        NavigationResult::Timeout => {
            ...
            let has_email_confirmation = BrowserHelpers::wait_for_page_content(page, "email", 1000).await? ||
            ...
            println!("[{:.2}s] Email confirmation message: {}", 
                     start_time.elapsed().as_secs_f64(),
                     if has_email_confirmation { "found" } else { "not found" });
        }
    }

    println!("[{:.2}s] ✓ Registration UI testing completed", start_time.elapsed().as_secs_f64());
    Ok(())
}

async fn test_login_ui(
    page: &Page,
    server_url: &str,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let form_submitted = BrowserHelpers::submit_form(page).await?;
    assert!(form_submitted, "Should be able to submit login form with valid username");
    ...
    match result {
        ...
        NavigationResult::Timeout => {
            ...
            let has_confirmation = BrowserHelpers::wait_for_page_content(page, "email", 1000).await? ||
               BrowserHelpers::wait_for_page_content(page, "sent", 1000).await? ||
               BrowserHelpers::wait_for_page_content(page, "magic", 1000).await? ||
               BrowserHelpers::wait_for_page_content(page, "link", 1000).await?;
            println!("Magic link confirmation: {}", if has_confirmation { "found" } else { "not found" });
        }
    }
...
async fn test_authenticated_workflow(
    ...
    let start_time = std::time::Instant::now();
    println!("[{:.2}s] Testing authenticated workflow with email confirmation", start_time.elapsed().as_secs_f64());
    
    // Register a fresh user for this test
    let test_email = "browser_test@example.com";
    let test_username = "browseruser";
    
    println!("[{:.2}s] Step 1: Register new user", start_time.elapsed().as_secs_f64());
    BrowserHelpers::navigate_and_wait(page, &format!("{}/register", server_url)).await?;
    BrowserHelpers::fill_registration_form(page, test_email, test_username).await?;
    
    let form_submitted = BrowserHelpers::submit_form(page).await?;
    assert!(form_submitted, "Should be able to submit registration form for authenticated test");
    println!("[{:.2}s] ✓ Registration form submitted", start_time.elapsed().as_secs_f64());
        
        // Step 2: Confirm registration via email
        println!("[{:.2}s] Step 2: Confirming registration via email", start_time.elapsed().as_secs_f64());
        BrowserHelpers::confirm_registration(page, server_url, smtp_port, test_email).await?;
        
        // Step 3: Test authenticated functionality
        println!("[{:.2}s] Step 3: Testing authenticated pages", start_time.elapsed().as_secs_f64());
        
        // Check if we're on a user entity page or redirected somewhere
        let current_url = page.url()?;
        println!("[{:.2}s] Current URL after confirmation: {}", start_time.elapsed().as_secs_f64(), current_url);
        
        // Test entity page navigation
        let entity_url = format!("{}/{}", server_url, test_username);
        println!("[{:.2}s] Navigating to entity page: {}", start_time.elapsed().as_secs_f64(), entity_url);
        BrowserHelpers::navigate_and_wait(page, &entity_url).await?;
        
        // Check if we can access the entity page
        let can_access_entity = BrowserHelpers::wait_for_page_content(page, test_username, 3000).await?;
        assert!(can_access_entity, "Should be able to access entity page after authentication");
        println!("[{:.2}s] ✓ Successfully accessed entity page", start_time.elapsed().as_secs_f64());
            
            // Test database creation
            println!("[{:.2}s] Step 4: Testing database creation", start_time.elapsed().as_secs_f64());
            
            // Look for database creation UI using single selector
            if BrowserHelpers::click_if_exists(page, "a[href*='create'], button:text('Create'), a:text('Create'), a:text('New Database')").await? {
                println!("[{:.2}s] ✓ Found and clicked database creation link", start_time.elapsed().as_secs_f64());
                // Wait for navigation to database creation page
                tokio::time::sleep(std::time::Duration::from_millis(1000)).await;
                
                // Try to fill database creation form
                let db_field_filled = BrowserHelpers::fill_field_if_exists(page, "input[name*='database'], input[name*='db_name'], input[name*='name']", "test_browser_db").await?;
                assert!(db_field_filled, "Should be able to fill database name field");
                println!("[{:.2}s] ✓ Filled database name field", start_time.elapsed().as_secs_f64());
                
                let db_form_submitted = BrowserHelpers::submit_form(page).await?;
                assert!(db_form_submitted, "Should be able to submit database creation form");
                
                tokio::time::sleep(std::time::Duration::from_millis(2000)).await;
                
                // Check if database was created successfully
                let db_created = BrowserHelpers::wait_for_page_content(page, "test_browser_db", 3000).await?;
                assert!(db_created, "Database 'test_browser_db' should be created successfully");
                println!("[{:.2}s] ✓ Database 'test_browser_db' created successfully", start_time.elapsed().as_secs_f64());
                
                // Test database page access
                let db_url = format!("{}/{}/test_browser_db", server_url, test_username);
                println!("[{:.2}s] Step 5: Testing database page access", start_time.elapsed().as_secs_f64());
                BrowserHelpers::navigate_and_wait(page, &db_url).await?;
                
                // Check if we can access the database page
                let can_access_db = BrowserHelpers::wait_for_page_content(page, "test_browser_db", 2000).await?;
                assert!(can_access_db, "Should be able to access database page after creation");
                println!("[{:.2}s] ✓ Successfully accessed database page", start_time.elapsed().as_secs_f64());
                
                // Test query interface with README.md example
                println!("[{:.2}s] Step 6: Testing query interface with README.md example", start_time.elapsed().as_secs_f64());
                ...
                let query_selector = "textarea[name*='query'], input[name*='query'], textarea[placeholder*='query'], textarea";
                ...
                println!("[{:.2}s] ✓ Found query input field", start_time.elapsed().as_secs_f64());
                                    
                                    // Execute each query from README.md
                                    for (i, query) in readme_queries.iter().enumerate() {
                                        println!("[{:.2}s] Executing query {}: {}", start_time.elapsed().as_secs_f64(), i + 1, query);
                                        
                                        // Fill and execute the query
                                        if BrowserHelpers::fill_field_if_exists(page, query_selector, query).await? &&
                                           BrowserHelpers::click_if_exists(page, "button:text('Run'), button:text('Execute'), input[type='submit'], button[type='submit']").await? {
                                                ...
                                                if query.starts_with("SELECT") {
                                                    ...
                                                    let complete_results = BrowserHelpers::verify_query_results(page, &expected_datasets, 2000).await?;
                                                    if complete_results {
                                                        ...
                                                        let has_partial = BrowserHelpers::wait_for_page_content(page, "PostgreSQL", 500).await? ||
                                                                         BrowserHelpers::wait_for_page_content(page, "SQLite", 500).await? ||
                                                                         BrowserHelpers::wait_for_page_content(page, "DuckDB", 500).await?;
                                                        if has_partial {
                                                            println!("[{:.2}s] ✓ Query {} executed with partial results visible", 
                                                                     start_time.elapsed().as_secs_f64(), i + 1);
                                                        } else {
                                                            println!("[{:.2}s] ✓ Query {} executed (results format may not be visible)", 
                                                                     start_time.elapsed().as_secs_f64(), i + 1);
                                                        }
                                                    }
                                                } else {
                                                    println!("[{:.2}s] ✓ Query {} executed successfully", 
                                                             start_time.elapsed().as_secs_f64(), i + 1);
                                                }
                                        }
                                    ...
                                    println!("[{:.2}s] ✓ Completed all README.md example queries", start_time.elapsed().as_secs_f64());
                                    
                                    // Test download functionality
                                    println!("[{:.2}s] Step 7: Testing download CSV and JSON functionality", start_time.elapsed().as_secs_f64());
                                    ...
                                    if BrowserHelpers::click_if_exists(page, "a[href*='csv'], button:text('CSV'), a:text('CSV'), a:text('Download CSV'), button:text('Download CSV')").await? {
                                        println!("[{:.2}s] ✓ Clicked CSV download button", start_time.elapsed().as_secs_f64());
                                        ...
                                        if let Ok(csv_content) = BrowserHelpers::find_and_read_download(&downloads_dir, "csv").await {
                                            println!("[{:.2}s] ✓ Found and read CSV download", start_time.elapsed().as_secs_f64());
                                            ...
                                            for (name, description, url) in &expected_csv_entries {
                                                if !csv_content.contains(name) || !csv_content.contains(description) || !csv_content.contains(url) {
                                            ...
                                            if all_entries_found {
                                                ...
                                                println!("    CSV preview: {}", csv_content.lines().take(4).collect::<Vec<_>>().join(" | "));
                                            } else {
                                                println!("[{:.2}s] ⚠ CSV content incomplete - missing: {}", start_time.elapsed().as_secs_f64(), missing_entries.join(", "));
                                                println!("    CSV content: {}", csv_content);
                                            }
                                        } else {
                                            println!("[{:.2}s] ⚠ CSV file not found in downloads", start_time.elapsed().as_secs_f64());
                                        }
                                    } else {
                                        println!("[{:.2}s] ⚠ No CSV download button found", start_time.elapsed().as_secs_f64());
                                    }
                                    
                                    // Test JSON download using single selector
                                    if BrowserHelpers::click_if_exists(page, "a[href*='json'], button:text('JSON'), a:text('JSON'), a:text('Download JSON'), button:text('Download JSON')").await? {
                                        println!("[{:.2}s] ✓ Clicked JSON download button", start_time.elapsed().as_secs_f64());
                                        
                                        // Wait for download to complete
                                        tokio::time::sleep(std::time::Duration::from_millis(2000)).await;
                                        
                                        // Look for JSON file in downloads
                                        if let Ok(json_content) = BrowserHelpers::find_and_read_download(&downloads_dir, "json").await {
                                            println!("[{:.2}s] ✓ Found and read JSON download", start_time.elapsed().as_secs_f64());
                                            ...
                                            for (name, description, url) in &expected_json_entries {
                                                if !json_content.contains(name) || !json_content.contains(description) || !json_content.contains(url) {
                                                    all_entries_found = false;
                                                    missing_entries.push(format!("{} ({}, {})", name, description, url));
                                                }
                                            }
                                                ...
                                                match serde_json::from_str::<serde_json::Value>(&json_content) {
                                                    Ok(json_value) => {
                                                        if let Some(array) = json_value.as_array() {
                                                            assert_eq!(array.len(), 3, "JSON should contain exactly 3 database records");
                                                            println!("[{:.2}s] ✓ JSON structure verified - {} records found (complete resultset)", 
                                                                     start_time.elapsed().as_secs_f64(), array.len());
                                                        }
                                                    },
                                                    Err(e) => {
                                                        println!("[{:.2}s] ⚠ JSON parsing failed: {}", start_time.elapsed().as_secs_f64(), e);
                                                    }
                                                }
                                            } else {
                                                println!("[{:.2}s] ⚠ JSON content incomplete - missing: {}", start_time.elapsed().as_secs_f64(), missing_entries.join(", "));
                                                println!("    JSON content: {}", json_content);
                                            }
                                        } else {
                                            println!("[{:.2}s] ⚠ JSON file not found in downloads", start_time.elapsed().as_secs_f64());
                                        }
                                    } else {
                                        println!("[{:.2}s] ⚠ No JSON download button found", start_time.elapsed().as_secs_f64());
                                    }
                                    
                                    println!("[{:.2}s] ✓ Download functionality testing completed", start_time.elapsed().as_secs_f64());
                                } else {
                                    println!("[{:.2}s] ⚠ No query input field found", start_time.elapsed().as_secs_f64());
                                }
                            } else {
                                println!("[{:.2}s] ⚠ Could not access database page", start_time.elapsed().as_secs_f64());
                            }
                        } else {
                            println!("[{:.2}s] ⚠ Database creation may not have succeeded", start_time.elapsed().as_secs_f64());
                        }
                    }
                } else {
                    println!("[{:.2}s] ⚠ No database name field found", start_time.elapsed().as_secs_f64());
                }
            } else {
                println!("[{:.2}s] ⚠ No database creation link found", start_time.elapsed().as_secs_f64());
            }
        } else {
            println!("[{:.2}s] ⚠ Could not access entity page - may not be authenticated", start_time.elapsed().as_secs_f64());
        }
    } else {
        return Err("Failed to submit registration form".into());
    }
    
    println!("[{:.2}s] ✓ Authenticated workflow testing completed", start_time.elapsed().as_secs_f64());
    Ok(())
}
...
async fn test_error_pages_ui(
    ...
    // Test 1: 404 page
    BrowserHelpers::navigate_and_wait(page, &format!("{}/nonexistent-page-12345", server_url)).await?;
    ...
    BrowserHelpers::click_if_exists(page, "a[href*='logout'], button:text('Logout'), a:text('Logout')").await.ok();
    tokio::time::sleep(std::time::Duration::from_millis(200)).await;
