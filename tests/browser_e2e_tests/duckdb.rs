...
pub async fn test_duckdb_flow(
    ...
    // Step 3: Submit and confirm we land on the DuckDB database page.
    ...
    // Step 7: Insert a third row and confirm the count is now three.
    run_query(
        page,
        "INSERT INTO test_table (fname, lname) VALUES ('snapshot', 'test');",
    )
    .await?;
    run_query(page, "SELECT COUNT(*) FROM test_table;").await?;
    let count_after_insert = page.locator("#query-results").await.inner_text().await?;
    assert!(
        count_after_insert.contains('3'),
        "Count after third insert should show 3 rows, got: {count_after_insert}"
    );
    ...
    // Step 10: Reload the database page and verify we're back to two rows
    // and the third ('snapshot') row is gone.
    page.goto(
    ...
    run_query(page, "SELECT COUNT(*) FROM test_table;").await?;
    let count_after_restore = page.locator("#query-results").await.inner_text().await?;
    assert!(
        count_after_restore.contains('2'),
        "Count after restore should show 2 rows, got: {count_after_restore}"
    );
