...
impl DbEngine for DuckdbEngine {
    ...
    fn create_snapshot(&self, db_path: &Path, snapshot_path: &Path) -> Result<(), AybError> {
        ...
        with_lock_retry(|| {
            let conn = duckdb::Connection::open_in_memory_with_flags(restricted_config()?)?;
    ...
    fn validate(&self, path: &Path) -> Result<(), AybError> {
        // DuckDB has no `PRAGMA integrity_check` equivalent, so the check
        // is "does this open as a DuckDB database and expose a readable
        // catalog?". The open uses `restricted_config` so that a file of
        // some *other* format (a SQLite database, say) fails here rather
        // than causing DuckDB to autoload a reader extension for it.
        let conn = with_lock_retry(|| {
            let config = restricted_config()?.access_mode(duckdb::AccessMode::ReadOnly)?;
            duckdb::Connection::open_with_flags(path, config)
        })
        .map_err(|err| AybError::Other {
            message: format!("File is not a valid DuckDB database: {err}"),
        })?;
        conn.execute_batch("SELECT count(*) FROM information_schema.tables;")
            .map_err(|err| AybError::Other {
                message: format!("File is not a readable DuckDB database: {err}"),
            })?;
        Ok(())
    }
}
...
/// Config for connections that open a database file directly from the
/// server process: snapshots and upload validation. Both run ATTACH or
/// open an arbitrary file, which needs external file access, so unlike
/// the query path we cannot disable it. Extension autoloading/
/// autoinstalling is not needed either way and is turned off here, in
/// the *open-time* config rather than with a post-open `SET`: DuckDB
/// sniffs a file's format as it opens it and will autoload a reader
/// extension (e.g. `sqlite_scanner`, fetched over the network from
/// extensions.duckdb.org) before any statement runs. These connections
/// live in the server process, outside the daemon's Landlock sandbox,
/// so that fetch would otherwise be unconstrained.
fn restricted_config() -> duckdb::Result<duckdb::Config> {
    base_config()?
